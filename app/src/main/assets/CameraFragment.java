...
package com.besttone.iar.fragment;
...
import com.besttone.iar.Consts;
import com.besttone.iar.R;
import com.besttone.iar.util.DensityUtils;
import com.besttone.iar.util.FileUtils;
import com.besttone.iar.util.LogUtils;
import com.besttone.iar.util.PictureUtils;
import com.besttone.iar.util.ViewUtils;
import com.up.spotlight.domain.metadata.Point;
...
public class CameraFragment extends UPFragment implements OnClickListener,
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		return super.init(inflater, container, savedInstanceState,
				R.layout.camera);
	}
	...
	public void openCamera() {
		if (!isSurfaceCreated) {
			LogUtils.Error(TAG, "surface is still not created!");
		}
		...
		try {
			...
			LogUtils.Debug(TAG, "picture size: " + optimalSize.width + "x"
					+ optimalSize.height);
			...
			LogUtils.Debug(TAG, "preview size: " + optimalSize.width + "x"
					+ optimalSize.height);
			...
			LogUtils.Debug(TAG, "surface position: " + lp.x + "," + lp.y);
			LogUtils.Debug(TAG, "surface size: " + lp.width + "," + lp.height);
	...
	private void setFlashMode(String mode) {
		...
		try {
			...
			LogUtils.Error(TAG, "IOException caused by setFlashMode", exception);
		}
	...
	public void onSurfaceTextureAvailable(SurfaceTexture surface, int width,
			int height) {
		LogUtils.Info(TAG, "surface texture is available");
		...
		try {
			...
			LogUtils.Error(TAG, "IOException caused by setPreviewTexture()",
					exception);
		}
	}
	...
	public void onSurfaceTextureSizeChanged(SurfaceTexture surface, int width,
			int height) {
		LogUtils.Info(TAG, "surface texture changed");
		if (mCamera != null) {
	...
	public boolean onSurfaceTextureDestroyed(SurfaceTexture surface) {
		LogUtils.Info(TAG, "surface texture destroyed");
					...
					public void run() {
						if (!hasAutoMode) {
							...
							if (setFocusArea(mPoint.getX(), mPoint.getY(), true)) {
								...
								AbsoluteLayout.LayoutParams params = (LayoutParams) mFocusView
										.getLayoutParams();
